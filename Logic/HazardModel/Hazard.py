...
def createEpicenter( momentum, x, y, parametros, objetos, modelohazard):
    ...
    cmd = "psql -d " + con.database + " -U " + con.user + maquina + " < " + sqld + "Hazard/create_epicenter_hazard2.txt > " + sql_temp + "errores2.txt"

    if (objetos == 1):
        cmd = "psql -d " + con.database + " -U " + con.user + maquina + " < " + sqld + "Hazard/create_epicenter_hazard1.txt > " + sql_temp + "errores1.txt"
